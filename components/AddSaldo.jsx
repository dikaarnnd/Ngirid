  ...
  const handleSave = async () => {
    ...
    if (!amount || !note) {
      const errorMessage = !amount && !note
        ? 'Jumlah saldo dan catatan harus diisi'
        : !amount
        ? 'Jumlah saldo harus diisi'
        : 'Catatan harus diisi';
      return Alert.alert('Data Belum Lengkap', errorMessage);
    }

    const saldoValue = parseInt(amount);
    if (saldoValue < 100) {
      return Alert.alert('Jumlah Tidak Valid', 'Jumlah saldo harus lebih dari Rp 100');
    }
          ...
          <View className="mx-3 gap-2">
            <View className="flex-row w-full gap-2">
              ...
              <TouchableOpacity
                ...
                onPress={() => setUserData((prev) => ({ ...prev, photoUrl: null }))}
              >
